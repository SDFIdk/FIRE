...
class BufferInMetersAroundPointsAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                "BufferOmValgtePunkter",
                "Buffer om valgte punkter",
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        outputs["ReprojektrLag"] = processing.run(
        ...
        if dan_concavehull:
            ...
            outputs["KonkavtHulAlfaform"] = processing.run(
                "qgis:concavehull",
                alg_params,
                context=context,
                feedback=feedback,
        ...
        style_file = os.path.join(
            os.path.dirname(__file__), "..", "styles", "buffer.qml"
        )
        alg_params = {"INPUT": outputs["Buffer"]["OUTPUT"], "STYLE": style_file}
        processing.run(
        ...
        alg_params = {"INPUT": outputs["Buffer"]["OUTPUT"], "NAME": "Buffer om punkter"}
        outputs["OmdbLag"] = processing.run(
            "native:renamelayer",
            alg_params,
            context=context,
            feedback=feedback,
