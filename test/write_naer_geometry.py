...
if __name__ == "__main__":
    ...
    fireDb = FireDb(db)
    output = open('output/write_near_geometry.xml','w')
    writer = GamaWriter(fireDb, output)

    g = Geometry("POINT (10.4811749340072 56.3061226484564)")
    os = fireDb.hent_observationer_naer_geometri(g, 10000)
    
    writer.take_observations(os)

    parameters = configparser.ConfigParser()
    parameters.read('fire-gama.ini')
    writer.write(True, False, "test/write_near_geometry.py", parameters)
    output.close    
